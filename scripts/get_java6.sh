...
export DEB_VERSION=`head -n1 "$BASE/src/$JAVA6/debian/changelog" | cut -d'(' -f2 | cut -d')' -f1 | cut -d'~' -f1`
export DEB_URGENCY=`head -n1 "$BASE/src/$JAVA6/debian/changelog" | cut -d'=' -f2`
...
ncecho " [x] $JAVA6: Getting Java SE download page "
...
if [ -n "${DOWNLOAD_INDEX}" ]; then
    ...
    ncecho " [x] $JAVA6: Failed to get current release page, getting previous releases download page "
    wget "http://www.oracle.com/technetwork/java/javasebusiness/downloads/java-archive-downloads-javase6-419409.html" -O "/tmp/oab-download-$JAVA6.html" >> "$LOG" 2>&1 &
    pid=$!;progress $pid
fi
...
do
    ...
    DOWNLOAD_URL=`grep "${JAVA_BIN}" "/tmp/oab-download-$JAVA6.html" | cut -d'{' -f2 | cut -d',' -f3 | cut -d'"' -f4`
    ...
    ln -s "$BASE/pkg/${JAVA_BIN}" "$BASE/src/$JAVA6/${JAVA_BIN}" >> "$LOG" 2>&1 &
    pid=$!;progress_loop $pid    
done
